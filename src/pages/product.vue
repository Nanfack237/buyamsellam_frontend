<template>
          ...
          <v-main style="height: 599px">
            ...
            <v-row class="px-7" >
                <v-card elevation="2" width="3000"  class="my-2 py-2 px-5">
                    ...
                    <v-data-table :items="products" id="printable-content" :headers="headers" :key="index" class="centered-headers" hide-default-footer :sort-by="[{ key: 'Product', order: 'asc' }]">
                      <template v-slot:item.actions="{ item }">
                        <v-btn icon size="small" class="mx-2" color="primary" @click="editProduct(item)">
                          <v-icon>mdi-pencil</v-icon>
                        </v-btn>
                        <v-btn icon size="small" onclick="confirm('do you really want to delete');" color="error" @click="deleteProduct(item)">
  ...
  async function fetchProducts() {
    try {
      ...
      products.value = response.data.products; // Assuming the API returns data in a 'products' property
    } catch (error) {
  ...
  try {
    const response = await axios.delete(`/api/products/delete/${item.id}`); // Replace with your actual endpoint
    if (response.status === 200) {
      ...
      console.error('Error deleting a deleting:', response.data.error);
    }
  } catch (error) {
