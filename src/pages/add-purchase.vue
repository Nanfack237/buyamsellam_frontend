          ...
          <v-row justify="space-between" class="d-flex justify-center px-5 pt-4 pb-2">
            <v-col cols="auto" class="">
              <p class="font-weight-bold text-h6 " style="color:#0D47A1;">
                Purchase Transaction <v-icon>mdi-package-variant-plus</v-icon>
              </p>
            </v-col>
            <v-col cols="auto">
          ...
          <form @submit.prevent="CreateStore">
            <v-container>
              <p class="text-medium-emphasis py-2 pb-5">
                Please enter the details to do a purchase transaction
              </p>
              <v-row>
                <v-col cols="12" sm="6" md="6" class="" style="padding-bottom: 0px;">
                  ...
                  <v-select label="Choose stock" v-model="selectedStock" :items="supplierLists" item-text="name"
                    item-value="id" variant="outlined" :rules="[
                ...
                <v-col cols="12" sm="6" md="6" class="" style="padding-bottom: 0px;">
                  <v-select label="Choose Supplier" v-model="selectedSupplier" :items="supplierLists" item-text="name"
                    item-value="id" variant="outlined" :rules="[
                ...
                <v-col cols="12" sm="" md="" class="" style="padding-top: 0px;">
...
async function fetchStocks() {
  try {
    const response = await axios.get('/api/suppliers'); // Replace with your actual endpoint
    const stocks = response.data.suppliers.map((stock) => stockLists.push({
      id: supplier.id,
      name: supplier.name
    })); // Extract titles
    suppliers.value = supplierNames;
...
const DoPurchase = async () => {
  ...
  try {
    ...
    formData.append('stock_id', stock_id.value);
