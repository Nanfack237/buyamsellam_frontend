                ...
                <div ref="printSection" class="d-none">
                  <div class="report-container">
                    ...
                    <div class="report-title-section">
                      ...
                      <p class="report-date">{{ $t('saleTransactionsVue.receipt.date') }}: {{ datePrint }}</p>
                    </div>
...
async function fetchSales() {
  try {
    ...
    sales.value = response.data.sales.map((sale: any) => {
      ...
      return {
        ...sale,
        productName: prod ? prod.name : t('saleTransactionsVue.tableCustomerN_A'), // Using N/A translation as placeholder
        costPrice: sto ? sto.cost_price : t('saleTransactionsVue.tableCustomerN_A'),
        cashierName: cashier ? cashier.name : t('saleTransactionsVue.tableCustomerN_A'),
      };
    });
  } catch (error) {
...
async function fetchStore() {
  try {
    const response = await axios.get('/api/stores/show', { headers: getAuthHeaders() });
    ...
    showSnackbar(t('saleTransactionsVue.snackbar.failedToLoadStoreInfo'), 'error');
  }
}
...
function getReceiptId(): string {
  ...
  return selectedSales.value[0].receipt_code.toString();
}
...
function downloadExcel() {
  ...
  if (link.download !== undefined) {
    ...
    link.setAttribute('download', `${t('saleTransactionsVue.title') }_${new Date().toISOString().split('T')[0]}.csv`);
